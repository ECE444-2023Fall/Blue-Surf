...
def setup_routes(app):
    ...
    def allowed_image(filename):
        # Add your own image file type validation here (e.g., check file extension)
    ...
    def upload_image():
        try:
            ...
            if image_file:
                # Validate the image file (e.g., check file type)
                if allowed_image(image_file.filename):
                    # Generate a unique filename for the uploaded image
                    image_filename = generate_unique_filename(image_file.filename)

                    # Save the image to the server's local storage
                    image_path = os.path.join(
                        app.config["UPLOAD_FOLDER"], image_filename
                    )
                    image_file.save(image_path)

                    # You can store the image path or filename in your database as needed
                    # Update the 'image' field in your event model with the image path or filename

                    return jsonify({"message": "Image successfully uploaded"})
                else:
                    return jsonify({"error": "Invalid file type for image"})
            else:
                return jsonify({"error": "No image data in the request"})
        except Exception as e:
            error_message = str(e)
            return (
                jsonify(
                    {"error": "Failed to upload image", "error message": error_message}
                ),
                500,
            )
